//determines what loaction you are at.
	...
	var ladle_checker 		= false;
	var silver_key_checker 	= false;
	var lockbox_checker 	= false;
	var keycard_checker 	= false;
	...
	function take_item() {
		...
		item_lockbox 	= new proto_item(2, "Lockbox", "This is a box shaped container, it is now open, and there was a small cube inside with a heart on each side.");
		...
		switch (currentlocation) {
			case 0: 
					if (ladle_checker === false) {
						...
						updatetext ("You already have the Ladle.");
						}
					...
					break;
			case 9:
					if (keycard_checker === false) {
						keycard_checker = true;
						
						inventoryArray[3] = item_keycard;
						updatetext ("Your inventory now contains the Keycard.");

					} else {
						updatetext ("You already have the Keycard.");
						}
					...
					break;
			case 8:
					if (silver_key_checker === true && lockbox_checker === false) {
						lockbox_checker = true;
						
						inventoryArray[2] = item_lockbox;
						updatetext ("Your inventory now contains the open Lockbox.  There was a small cube inside.");

					} else {
						if (silver_key_checker === false) {
						updatetext ("you can't open the Lockbox, you are missing the key.")
							} 
						if (silver_key_checker === true){
							updatetext ("You already have the Lockbox.");
							}
						}
					...
					break;
			case 7: 
					if (ladle_checker === true && silver_key_checker === false) {
						silver_key_checker = true;
						...
						if (silver_key_checker === false) {
						updatetext ("you can't reach the Silver Key you will burn your hand.  You should find something to grab it.");
							} 
						if (silver_key_checker === true) {
							updatetext ("You already have the Silver Key.");
							}
						}
	...
	function creating_locations() {
  		...
  		location2 = new proto_location("2", "Facility Entrance", "You walk into a room with tinted glass walls on all sides.  There are empty shelves on the floor and a metal door across the room that is locked by a keycard device, and a small table that has a cube shaped slot carved into it.", "");
  		...
  		location10 = new proto_location("10", "Testing Chamber", "You find yourself in a chamber that has many different walls, and in the center of the room there is a item that looks a lot like a gun, but it is white and large.", "");
	...
	function btn_move_click(direction) {
		...
		if (newlocation === 10 && lockbox_checker === true && keycard_checker === true) {
			updatetext ("You put the cube on the slot in the table, and the put the keycard in the device.  The door opened and you find yourself in a chamber that has many different walls, and in the center of the room there is a item that looks a lot like a gun, but it is white and large." + "\n" + "\n" + "CONGRATULATIONS!  You made it to the testing chambers, now its time for the real experiment to begin... (to be continued.) To play again please refresh the web page.  Thanks for playing.");
				...
				document.getElementById("txtcommand").style.visibility="hidden";
				document.getElementById("enterbutton").style.visibility="hidden";
					...
					if (newlocation === 10) {
						updatetext("The door is locked.  You might be missing something.")
						} else {
							if (newlocation !== -1) {
	...
	function btn_score_click() {
		updatetext("You have " + score + " points.");
			}											
